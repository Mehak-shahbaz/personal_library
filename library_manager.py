...
import json
import osfrom datetime import datetime # type: ignore
import time
import random
...
def add_book(title, author, publication_year,genre, read_status):
    book = {
        ...
        'added_date':datetime.now().strfime("%Y-%m-%d-%H:%M:%S")
    }
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.book_removed:
        st.markdown("div class='success-message'> Book removed sucessfully!", unsafe_allow_html=True)
        st.session_state.book_removed = False
    ...
    if hasattr(st.session_state, 'state_results'):
...
st.markdown("---")
st.markdown("Copyright @ 2025 Mehak Shahbaz Personal Library Manager", unsafe_allow_html=True)
